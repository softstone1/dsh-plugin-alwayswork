---
name: anakut-bootstrap
description: Turn this machine into a managed Anakut Worker. Use when asked to set up, enroll or onboard this box as an Anakut worker, or when an Anakut control-plane URL and join token are provided.
---

# Anakut Worker bootstrap

Goal: this machine becomes a managed Anakut Worker with no manual follow-up.
...
3. **Stay managed.** `sudo aw enable control.join` installs the systemd
   ...
   `systemctl status anakut-worker-agent`.
...
one-line installer from `softstone1/anakut-worker`.

## Behaviour rules

- Prefer the `anakut_*` tools when available; otherwise use `aw` in the shell.
...
- Desired state is authored by the control plane. Do not hand-edit
  `/etc/anakut-worker/worker.yaml` to work around it; fix the group in the
...
- `anakut-worker-agent.service` is active and enabled.
